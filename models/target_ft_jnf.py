...
class Target_FT_JNF(BaseModel):
    ...
    def __init__(self,
                 ...
                 n_freqs: int = 257,
                 n_channels: int = 2,
                 n_lstm_hidden1: int = 80, #std: 64/16
        ...
        :param n_lstm_hidden2: numder of LSTM units in the second LSTM layer
        ...
        """
        super(Target_FT_JNF, self).__init__()
        ...
        if self.with_regressor:
            self.regressor_lstm1 = torch.nn.Conv2d(in_channels=self.n_lstm_hidden1,
                                                 ...
                                                 stride=1,
                                                 padding=0,)
            self.regressor_lstm2 = torch.nn.Conv2d(in_channels=self.n_lstm_hidden2,
                                                 out_channels=128,
                                                 kernel_size=1,
                                                 stride=1,
                                                padding=0)
    ...
    def forward(self, x: torch.Tensor, return_mask: bool=False, return_layer: bool=False):
        ...
        x, _ = self.lstm1(x)
        if return_layer:
            #regressor_in = x.detach().clone().permute(2,1,0).unsqueeze(0)
            #self.lstm1_layer_out = self.regressor_lstm1(regressor_in) if you use regressor!
            lstm1_layer_out = x
        x = self.dropout(x)
        ...
        if return_layer:
            #regressor_in = x.detach().clone().permute(2,1,0).unsqueeze(0)
            #self.lstm2_layer_out = self.regressor_lstm2(regressor_in) if you use regressor!
            lstm2_layer_out = x
        x = self.dropout(x)
        ...
        if return_layer:
            return self.inverse_spec_transform(y, length=time_length), lstm1_layer_out, lstm2_layer_out, linear_layer_out
...
if __name__ == '__main__':
    ...
    mod = Target_FT_JNF(output_type='Nch_CRM', n_channels=3, channels_to_filter=[0, 1, 2],
