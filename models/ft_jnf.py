...
STANDARD_DEVICE = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')


class FT_JNF(BaseModel):
    """
    ...
    def forward(self, x: torch.Tensor, return_mask: bool=False, return_layer: bool=False):
        ...
        x, _ = self.lstm1(x)
        if return_layer:
            LSTM1_out = x #.permute(2,1,0).unsqueeze(0) if you use regressor on the student model
        x = self.dropout(x)
        ...
        x, _ = self.lstm2(x)
        if return_layer:
            LSTM2_out = x #.permute(2,1,0).unsqueeze(0) if you use regressor on the student model
        x = self.dropout(x)
        ...
        if return_layer:
            return self.inverse_spec_transform(y, length=time_length), LSTM1_out, LSTM2_out, linear_out
...
if __name__ == '__main__':
    ...
    mod = FT_JNF(output_type='Nch_CRM', n_channels=3, channels_to_filter=[0, 1, 2],
